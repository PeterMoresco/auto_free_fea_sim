...
def run_test(install, max_count, ca_path):
    ...
    # Run the testcases
    RUN_TEST(install, as_run, tmp_dir, test_cases, max_count)
    # Diagnose the results
    RUN_DIAGNOSE(as_run, tmp_dir)
    # Finishing job
...
def RUN_TEST(install, as_run, tmp_dir, testcases, m_count):
...
def RUN_DIAGNOSE(as_run, tmp_dir):
    click.echo(click.style('Running diagnoses', fg='cyan'))
    os.system('{} --diag --only_nook --astest_dir={} > {}diag'.format(
        as_run, tmp_dir, tmp_dir))
    os.system('cat {}diag'.format(tmp_dir))
